...
select total.*, case_onset, confirmed_cases, male from
	(select cdc_report_dt, count(*) as total_cases from covid_19_us 
	group by cdc_report_dt) as total
join
...
join
	...
	group by cdc_report_dt) as confirmed
on total.cdc_report_dt = confirmed.cdc_report_dt
join
	(select cdc_report_dt, count(*) as male from covid_19_us
	where sex = "Male"
	group by cdc_report_dt) as male
on total.cdc_report_dt = male.cdc_report_dt;
